...
#{apt} install alsa-utils pulseaudio
#{apt} install python3-yaml python3-schema python3-pyside2.qtcore python3-pyside2.qtgui python3-pyside2.qtwidgets python3-gst-1.0 gir1.2-gtk-3.0
#{apt} install -t testing gstreamer1.0-plugins-base
adduser vagrant audio
amixer set Master 100%
amixer set Master unmute
amixer set PCM 100%
...
#{apt} update
#{apt} full-upgrade
#{apt} install alsa-utils pipewire
#{apt} install python3-yaml python3-schema python3-pyside2.qtcore python3-pyside2.qtgui python3-pyside2.qtwidgets python3-gst-1.0 gir1.2-gtk-3.0
#{apt} install gstreamer1.0-plugins-base
adduser vagrant audio
amixer set Master 100%
...
#{apt} install alsa-utils pulseaudio
#{apt} install python3-yaml python3-schema python3-pyside2.qtcore python3-pyside2.qtgui python3-pyside2.qtwidgets python3-gst-1.0 gir1.2-gtk-3.0
#{apt} install -t bullseye gstreamer1.0-plugins-base
adduser vagrant audio
amixer set Master 100%
amixer set Master unmute
amixer set PCM 100%
amixer set PCM unmute
  ...
  config.vm.define "debtesting3" do |conf|
    # debian testing with pulse from stable
    conf.vm.box = "debian/testing64"
    conf.vm.provision "shell", inline: <<-SHELL
set -e
echo "deb https://deb.debian.org/debian bullseye main non-free contrib" >> /etc/apt/sources.list
...
#{apt} install -t bullseye pulseaudio
#{apt} install python3-yaml python3-schema python3-pyside2.qtcore python3-pyside2.qtgui python3-pyside2.qtwidgets python3-gst-1.0 gir1.2-gtk-3.0
#{apt} install gstreamer1.0-plugins-base
adduser vagrant audio
amixer set Master 100%
